...
The Fourier Transform is a common tool in signal processing. It transforms a time based signal to the frequency domain showing the frequency components contained in the signal as a complex number, which is often displayed as magnitude and phase.

A property of the Fourier Transform of interest is the convolution theorem which states that convolution in time is multiplication in frequency \ref{eq:ConvolutionMultiplication}. 
\inote{Define the Fourier Transform}
...
\int\limits_{-\infty}^{\infty}f(t)g(t-\tau)d\tau = f(t) \ast g(t) = X(f)\cdot Y(f)
\end{equation}
\inote{example of convolution used as autocorrelation. Include some pretty graphs}
