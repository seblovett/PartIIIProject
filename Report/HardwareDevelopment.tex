...
As the voltage swing on the collector of the phototransistor does not reach near 0V, the AVR cannot detect this as a logical 0. An external amplifier could be used to generate a full swing voltage. However, the AVR has ADCs and analogue comparators. Using the ADC has the advantage of some signal processing to filter the data, but this is processor hungry and time consuming.
...
A second approach utilised a simple state machine and software based hysteresis to solve the problems. 
\inote{Insert ASM picture and explain away!}
