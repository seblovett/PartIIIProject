...
After the data has been read, the buffer is reset by asserting the read and write reset signals (RRST and WRST) for at least one clock cycle of RCLK or PCLK respectively. The entire operation can be seen in figure \ref{fig:ov_Capture}.
\begin{figure}
...
\inote{From Tom: Sorry for so many notes, you should be able to see my points of understanding through that, it is good and I pretty much get it (I wish we knew for D4 haha) but there are a few ambiguities and stuff that don't help. Maybe a little diagram with Camera, FIFO, and Microcontroller on it showing the signals between them would help aswell, but it isn't that necessary.}
...
The Eagle CAD Diagram of the PCB can be seen in Appendix \ref{Appendix:PCB}. The PCB was manufactured by \cite{PCBCart}. The PCB cost \pounds 205 to manufacture and ship five PCBs. A photo of the PCB can be seen in figure \ref{fig:PCB:Bare}. 
...
\begin{figure}
\centering
\subfigure[Top of the PCB]{\includegraphics[scale=0.4]{./Figures/PCB_Bare_Top.jpg} }
\subfigure[Bottom of the PCB]{\includegraphics[scale=0.4]{./Figures/PCB_Bare_Bottom.jpg} }
